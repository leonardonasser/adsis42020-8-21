package aula20200821.LeonardoNasser.fork;
...
public class ImóvelRepository {
	...
	public Imóvel findById(Long id) throws SQLException {
		...
		try {
			if (rsSelect.next()) {
				found = new Imóvel(rsSelect.getLong("id"), rsSelect.getString("descricao"),
	...
	public List<Imóvel> findAll() throws SQLException {
		...
		try {
			while (rsSelect.next()) {
				Imóvel imóvel = new Imóvel(rsSelect.getLong("id"), rsSelect.getString("descricao"),
						rsSelect.getDouble("metragem"));
				all.add(imóvel);
			}
		} finally {
